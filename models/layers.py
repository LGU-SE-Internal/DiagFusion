...
class GATClassifier(nn.Module):
    ...
    def __init__(self, in_dim, hidden_dim, n_classes, num_heads):
        ...
        self.conv1 = dglnn.GATConv(
            in_dim, hidden_dim, num_heads=num_heads, activation=F.relu
        )  # F.relu
        self.conv2 = dglnn.GATConv(
            hidden_dim * num_heads, hidden_dim, num_heads=num_heads, activation=F.relu
        )
        # self.pool = dglnn.AvgPooling()
        self.pool = dglnn.MaxPooling()
        self.classify = nn.Linear(hidden_dim, n_classes)
...
class SAGEClassifier(nn.Module):
    ...
    def __init__(self, in_dim, hidden_dim, n_classes):
        ...
        self.conv1 = dglnn.SAGEConv(
            in_dim, hidden_dim, "lstm", activation=F.relu
        )  # mean, gcn, pool, lstm

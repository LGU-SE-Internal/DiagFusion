...
class InvocationEvent:
    def __init__(self, cases, data_dir, dataset, trace_pairs_path, config=None):
        ...
        self.res = self.res = dict(
            zip(list(cases.index), [[] for _ in range(len(cases))])
        )
            ...
            for case_id, case in tqdm(self.cases.iterrows()):
                ...
                for caller, callee in self.pairs:
                    ...
                    res1 = self.detector.detection(
                        invocation_data, "lagency", start_ts, end_ts
                    )
            ...
            for case_id, case in tqdm(self.cases.iterrows()):
                ...
                for caller, callee in self.pairs:
                    temp_csv = os.path.join(
                        self.data_dir, day, f"{caller}_{callee}.csv"
                    )
                    ...
                    res2 = self.detector.detection(
                        invocation_data, "other", start_ts, end_ts
                    )
                    #                 print(res1, res2)
                    if not (res1[0] or res2[0]):
                        continue
...
if __name__ == "__main__":
    ...
    invocation_event = InvocationEvent(
        labels, trace_data_dir, config["dataset"], config["trace_pairs_path"]
    )
    invocation_event.get_invocation_events()

    save_path = os.path.abspath(
        os.path.join(project_root_dir, pf.deal_config(config, "parse")["trace_path"])
    )

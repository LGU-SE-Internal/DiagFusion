    ...
    for batch_idx in range(0, len(paths), batch_size):
        ...
        for case_id, data_pack in enumerate(batch_paths, start=batch_idx):
            try:
                ...
                batch_traces.append(
                    abnormal_trace_df[["service_name", "parent_service"]]
                )

                # 使用 k_sigma 计算异常分数
                k_sigma = Ksigma()
                scores = []
                for name, group in abnormal_trace_df.groupby(
...
def generate_service_topology(all_traces):
    ...
    if all_traces:
        ...
        # 服务ID到服务名称的映射
        service_id_to_name = {
            idx: service_name for idx, service_name in enumerate(services)
        }

        # 服务名称到ID的映射
        service_to_id = {service_name: idx for idx, service_name in enumerate(services)}
        # 创建实例到ID的映射
        ...
        with open(topology_path, "w") as f:
            json.dump(
                {"source_nodes": topology[0], "target_nodes": topology[1]}, f, indent=4
            )
...
def _build_invoke_links(df):
    ...
    df["invoke_link"] = df["parent_service"].fillna("ROOT") + "_" + df["service_name"]
...
def service_to_instance(
    topology=None, gt_df=None, service_id_to_name=None, instance_to_id=None
):
    """将服务级别的拓扑转换为实例级别的拓扑
    ...
    for service_id, service_name in service_id_to_name.items():
        instance_ids = [
            instance_to_id[inst] for inst in service_to_instances[service_name]
        ]

...
def get_fault2templates(fault_file, mid_dir):
    ...
    for path in paths:
        ...
        for idx, row in faults.iterrows():
            ...
            execute_time = int(
                time.mktime(time.strptime(execute_time_str, "%Y-%m-%d %H:%M:%S"))
            )
            ...
            fault2template_file = (
                f"{mid_dir}fault2template/fault_{execute_time}_{_id}.csv"
            )
            ...
            print(
                f"[get_fault2templates] execute_time: {execute_time} \

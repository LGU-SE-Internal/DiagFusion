...
class LogEvent:
        ...
        for i in range(0, len(labels)):
            ...
            for j in range(len(df)):
                ...
                for k in range(duration):
                    ...
                    temp_df = df[j].loc[
                        (df[j]["timestamp"] >= (st_stamp + k * 60000))
        ...
        for i in range(0, len(labels)):
            ...
            for j in range(len(df)):
                ...
                for k in range(len(cnt)):
                    if cnt[k] == 1:
                        unique_time = temp_df["datetime"].loc[
                            temp_df["EventId"] == event_id[k]
                        ]
    ...
    def do_lab(self):
        ...
        df_1 = np.load(
            r"D:\Projects\unirca\data\gaia\source_data\log\df_1.npy", allow_pickle=True
        )
        df_2 = np.load(
            r"D:\Projects\unirca\data\gaia\source_data\log\df_2.npy", allow_pickle=True
        )
        df_3 = np.load(
            r"D:\Projects\unirca\data\gaia\source_data\log\df_3.npy", allow_pickle=True
        )
        df = list(df_1) + list(df_2) + list(df_3)
